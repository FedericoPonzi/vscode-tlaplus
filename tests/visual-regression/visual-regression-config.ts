/**
 * Configuration for permanent visual regression testing infrastructure
 * This configuration is used for ongoing visual regression testing after the migration
 */
...
export const DEFAULT_CONFIG: VisualRegressionConfig = {
    ...
    paths: {
        baseline: 'tests/visual-regression/screenshots/',
        current: 'tests/visual-regression/current/',
        diff: 'tests/visual-regression/diff/',
...
/**
 * Critical test scenarios that must pass for any UI changes
 */
...
/**
 * Map screenshot filenames to test scenarios
 */
export function getScenarioFromFilename(filename: string): TestScenario | undefined {
    // Extract components from filename
    const parts = filename.replace('.png', '').split('_');
    if (parts.length < 5) {return undefined;}

    const viewName = parts[0] + '-' + parts[1] + '-' + parts[2];
    const theme = parts[3];
    const dimensions = parts[4].split('x');
    const zoomLevel = parseInt(parts[5].replace('pct', ''));
    const description = parts.slice(6).join('-');

    return TEST_SCENARIOS.find(scenario =>
        scenario.viewName === viewName &&
    scenario.theme === theme &&
    scenario.viewport.width === parseInt(dimensions[0]) &&
    scenario.viewport.height === parseInt(dimensions[1]) &&
    scenario.zoomLevel === zoomLevel &&
